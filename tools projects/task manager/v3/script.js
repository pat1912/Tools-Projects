            ...
            confirmDialog.innerHTML = `
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <span class="close-button" id="closeConfirmDialog">&times;</span>
                    </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button class="save-button danger-button" id="confirmAction">Confirm</button>
                    </div>
                </div>
            `;
...
handleAddNote() {
   ...
   // New custom alert method
   showAlert(message) {
       // Create alert dialog if it doesn't exist
       ...
       if (!alertDialog) {
           ...
           alertDialog.innerHTML = `
               <div class="modal-content">
                   <div class="modal-header">
                       <h2>Alert</h2>
                       <span class="close-button" id="closeAlertDialog">&times;</span>
                   </div>
                   <div class="modal-body">
                       <p id="alertDialogMessage"></p>
                   </div>
                   <div class="modal-footer">
                       <button class="save-button" id="okAlert">OK</button>
                   </div>
               </div>
           `;
   ...
   handleCopyReport() {
       ...
       navigator.clipboard.writeText(reportText).then(() => {
           this.showAlert('Report copied to clipboard!');
       }, (err) => {
   ...
   handleImportTasks() {
       ...
       if (file.type !== 'application/json') {
           this.showAlert('Please select a valid JSON file');
           return;
       }
       ...
       reader.onload = (e) => {
           try {
               ...
               if (importedData.tasks && importedData.categories) {
                   ...
                   this.showConfirmDialog(
                       'Import Data',
                       `Import ${importedData.tasks.length} tasks and ${importedData.categories.length} categories? This will replace your current data.`,
                       () => {
                   ...
                   this.showConfirmDialog(
                       'Import Tasks',
                       `Import ${importedData.length} tasks? This will replace your current tasks.`,
                       () => {
               ...
               this.showAlert('Error importing data. Please check that the file is a valid task backup.');
           }
       };
   ...
   handleEditCategory(id) {
       ...
       // Create edit category dialog if it doesn't exist
       let editCategoryDialog = document.getElementById('editCategoryDialog');
       if (!editCategoryDialog) {
           editCategoryDialog = document.createElement('div');
           editCategoryDialog.id = 'editCategoryDialog';
           editCategoryDialog.className = 'modal';
           
           editCategoryDialog.innerHTML = `
               <div class="modal-content">
                   <div class="modal-header">
                       <h2>Edit Category Name</h2>
                       <span class="close-button" id="closeEditCategoryDialog">&times;</span>
                   </div>
                   <div class="modal-body">
                       <input type="text" id="editCategoryNameInput" placeholder="New category name...">
                       <input type="hidden" id="editCategoryId">
                   </div>
                   <div class="modal-footer">
                       <button class="cancel-button" id="cancelEditCategory">Cancel</button>
                       <button class="save-button" id="saveEditCategory">Save</button>
                   </div>
               </div>
           `;
           
           document.body.appendChild(editCategoryDialog);
           
           // Add event listeners
           document.getElementById('closeEditCategoryDialog').addEventListener('click', () => {
           ...
           document.getElementById('saveEditCategory').addEventListener('click', () => {
               ...
               if (newName) {
                   ...
                   if (category) {
                       this.categoryManager.updateCategory(categoryId, newName, category.color);
                       this.renderCategories();
                       this.populateCategoryDropdowns();
                   }
               } else {
           ...
           // Add input event listener for text direction
           document.getElementById('editCategoryNameInput').addEventListener('input', function() {
       ...
       detectTextDirection(document.getElementById('editCategoryNameInput'));
       
       // Show dialog
       editCategoryDialog.style.display = 'flex';
   }
   ...
   handleDeleteCategory(id) {
       ...
       if (tasksUsingCategory.length > 0) {
           this.showConfirmDialog(
               'Delete Category',
               `This category is used by ${tasksUsingCategory.length} tasks. Do you want to remove the category from these tasks and delete it?`,
               () => {
                   ...
                   if (this.categoryManager.deleteCategory(id)) {
                       ...
                       this.showAlert('Default categories cannot be deleted.');
                   }
               }
           );
       } else {
