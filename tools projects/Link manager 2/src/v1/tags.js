...
function initializeTagInputs() {
  ...
  // For editing link - similar implementation
  const editTagInput = document.getElementById('editLinkTags');
  const editTagsDisplay = document.getElementById('editTagsDisplay');
  
  if (editTagInput && editTagsDisplay) {
    // Initialize selected tags array for edit form
    window.editSelectedTags = [];
    
    editTagInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ',') {
        e.preventDefault();
        const tagName = editTagInput.value.trim();
        if (tagName && !window.editSelectedTags.includes(tagName)) {
    ...
    // Modify edit form submission to include tags
    ...
    if (editLinkForm) {
      editLinkForm.addEventListener('submit', (e) => {
        ...
        if (!link) {
          ...
          elements.editLinkModal.style.display = 'none';
          return;
        }
        ...
        const groupId = document.getElementById('editLinkGroup').value;
        const remarks = document.getElementById('editLinkRemarks').value;
        
        // Update link object
        link.url = url;
        link.title = title;
        link.groupId = groupId || null;
        link.remarks = remarks;
        
        // Handle tag updates - first update tag counts for removed tags
        if (link.tags) {
          link.tags.forEach(oldTag => {
            if (!window.editSelectedTags.includes(oldTag)) {
              // Tag was removed
              const tag = state.tags.find(t => t.name.toLowerCase() === oldTag.toLowerCase());
              if (tag) {
                tag.linkCount--;
                if (tag.linkCount <= 0) {
                  state.tags = state.tags.filter(t => t.id !== tag.id);
        ...
        // Then update tag counts for added tags
        window.editSelectedTags.forEach(newTag => {
          if (!link.tags || !link.tags.includes(newTag)) {
            // Tag was added
            const tag = createOrGetTag(newTag);
            if (tag) tag.linkCount++;
          }
        });
        
        // Update link's tags
        link.tags = [...window.editSelectedTags];
...
function updateEditTagsDisplay() {
  const editTagsDisplay = document.getElementById('editTagsDisplay');
  if (!editTagsDisplay) return;
  
  editTagsDisplay.innerHTML = '';
  window.editSelectedTags.forEach(tagName => {
    const tagPill = document.createElement('span');
    tagPill.className = 'tag-pill';
